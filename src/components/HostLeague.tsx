        ...
        <Field label="Battle gimmick">
          <div className="grid grid-cols-2 gap-2">
            {GENERATIONS.map((g) => (
              ...
              >
                <div className="font-display font-bold">{g.gimmick}</div>
                <div className="text-xs text-ink-soft">{g.gen === 6 ? "Mega Stones only" : `${g.label} · also allows Mega`}</div>
              </button>
            ))}
          </div>
          <p className="text-xs text-ink-soft mt-1">Mega Evolution works in every option (hold a Mega Stone). “Mega” disables the other gimmicks.</p>
        </Field>
